...
`n_body_sim_pro_threading_set_thread_count(n)` configures the active thread count;
...
default and `n_body_sim_pro_threading_openmp_available()` returns 0.
...
`n_body_sim_pro_thread_affinity_apply_policy`.
